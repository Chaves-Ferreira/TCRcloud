...
def aminoacids(args):
	...
	if args.by_length:
		...
		total_reads = samples_df["counts"].sum() if "counts" in samples_df.columns else len(samples_df)
		# Process each length group (filtering groups with <2.5% of total reads)
		for length, group in samples_df.groupby("cdr3_length"):
			group_reads = group["counts"].sum() if "counts" in group.columns else len(group)
			if group_reads / total_reads < 0.025:
				sys.stderr.write(f"Skipping CDR3 length {length} due to low read count.\n")
				continue
			...
			for j in keys:
				...
				if args.compare.lower() == "false":
					...
					normalized = normalized.reindex(all_aa.index, fill_value=0) # fill missing a.a value to 0 if absent
					transposed_df = normalized.transpose() # Convert for plotting, for 2D bar plot
					...
					if args.threeD.lower() == "false":
						...
						outputname = (
							args.rearrangements[:-4] + "_aminoacids_" + j[1] + "_" + j[0] + f"_L{length}.png"
						)
					...
					# If user wants single sample 3D plot
						...
						x_ordered = [aa for aa in desired_order if aa in x] if 'x' in locals() else desired_order
			...
			if args.compare.lower() == "true":
					...
					for diff_df, label in comparisons:
						...
						for i in range(num_x):
							for j in range(num_y):
								...
								z_max = z_val
								mesh_list.append(
									generate_mesh(x_min, x_max, y_min, y_max, z_min, z_max, cval)
								)
				...
				# If user wants a 2D difference
					...
					for diff_df, label in comparisons:
						...
						comp_T = comp_T.loc[:, comp_T.columns.str.strip() != ""] # Drop empty column if present
						...
						for aa in desired_order:
							if aa in comp_T.columns:
								...
								# Positive bars
								ax.bar(positions, pos_values, bottom=cum_pos,
									color=colours[aa],
									label=aa if aa not in ax.get_legend_handles_labels()[1] else "")
								cum_pos += pos_values

								# Negative bars
								ax.bar(positions, neg_values, bottom=cum_neg,
									color=colours[aa])
								cum_neg += neg_values
